 ...
 * to "1=1" and lib/* fetches the full dataset and aggregates in JS exactly as
 ...
 * (the demo user is state-level, so real scope filters are "1=1" and lib/*
 * filters/aggregates in JS anyway). COUNT(*) queries return `cnt`; a query
 * carrying both a `>=` and a `<` date bound is treated as a "prior period"
 * and down-weighted so the dashboard's trend delta reads sensibly.
 */
export function demoQuery(query, tableName) {
  ...
  if (q.includes("COUNT(*)")) {
    ...
    return [{ cnt: isPriorWindow ? Math.round(rows.length * 0.4) : rows.length }];
  }
  return rows.map((r) => ({ ...r }));
}
