...
// The Data Store column names follow the KSP ER diagram (e.g. CrimeHead's name
// column is `CrimeGroupName`, and CrimeSubHead's is confusingly `CrimeHeadName`).
// These queries alias them to the names the rest of the app already consumes,
// so the ER schema stays canonical without churning every call site.
export function getCrimeHeads() {
  return memoizePerRequest("lookup:crimeHeads", () =>
    zcqlQuery(`SELECT ROWID AS CrimeHeadID, CrimeGroupName AS CrimeHeadName FROM CrimeHead`, "CrimeHead")
  );
}

export function getCrimeSubHeads() {
  return memoizePerRequest("lookup:crimeSubHeads", () =>
    zcqlQuery(`SELECT ROWID AS CrimeSubHeadID, CrimeHeadName AS CrimeSubHeadName FROM CrimeSubHead`, "CrimeSubHead")
  );
}

export function getGravityOffences() {
  return memoizePerRequest("lookup:gravityOffences", () =>
    zcqlQuery(`SELECT ROWID AS GravityOffenceID, LookupValue AS GravityName FROM GravityOffence`, "GravityOffence")
  );
}

export function getCaseCategories() {
  return memoizePerRequest("lookup:caseCategories", () =>
    zcqlQuery(`SELECT ROWID AS CaseCategoryID, LookupValue AS CategoryName FROM CaseCategory`, "CaseCategory")
  );
}

export function getCaseStatuses() {
  return memoizePerRequest("lookup:caseStatuses", () =>
    zcqlQuery(`SELECT ROWID AS CaseStatusID, CaseStatusName AS StatusName FROM CaseStatusMaster`, "CaseStatusMaster")
  );
}
