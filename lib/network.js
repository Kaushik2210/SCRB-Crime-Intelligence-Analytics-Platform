...
export async function getNetworkGraph(session, maxOffenders = 30) {
  ...
  const [accusedRows, cases, subHeads] = await Promise.all([
    zcqlQuery(
      // ER-diagram columns (AccusedName / GenderID) aliased to the names the
      // identity-matching below uses. GenderID is a numeric lookup code — only
      // its equality matters here, not its label.
      `SELECT ROWID AS AccusedMasterID, AccusedName AS Name, GenderID AS Gender, CaseMasterID FROM Accused
