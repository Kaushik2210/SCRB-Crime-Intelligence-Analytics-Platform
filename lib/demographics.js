...
async function groupByCaseIds(complainants, dimension) {
  ...
  if (dimension === "caste") {
    ...
    const castes = await zcqlQuery(
      `SELECT ROWID AS CasteID, caste_master_name AS CasteName FROM CasteMaster`,
      "CasteMaster"
    );
...
export async function getAggregateDemographics(caseMasterIds) {
  ...
  // Catalyst ROWIDs exceed Number.MAX_SAFE_INTEGER and are handled as strings
  // throughout, so `Number.isInteger` would reject every real id. Validate the
  // digits-only shape instead — this is still an injection guard, since these
  // ids are spliced straight into the IN (...) list below.
  const safeIds = caseMasterIds.filter((id) => /^\d+$/.test(String(id)));
