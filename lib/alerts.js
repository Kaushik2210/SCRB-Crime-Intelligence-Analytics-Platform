...
import { getUnits, getDistricts } from "@/lib/lookups";
import { toDbDateTime } from "@/lib/dbDate";

/** ChargesheetDetails.cstype is a single-char final-report code (ER diagram). */
...
export async function getAlertsFeed(session) {
  ...
  const [anomalies, chargesheetRows, units, districts, actions] = await Promise.all([
    ...
    zcqlQuery(
      // ER-diagram column names (cstype / csdate) aliased to what this file's
      // consumers already read.
      `SELECT ROWID AS ChargesheetID, CaseMasterID, cstype AS ReportType, csdate AS ReportDate
       FROM ChargesheetDetails
       WHERE CaseMasterID IN (SELECT ROWID FROM CaseMaster WHERE ${scopeFilter})
       ORDER BY csdate DESC
       LIMIT 40`,
      "ChargesheetDetails"
    ),
    getUnits(),
  ...
  const chargesheetItems = chargesheetRows
    .map((c) => {
      ...
      return {
        ...
        message: `Case ${caseRow.CrimeNo} at ${unit.UnitName} was marked "${CSTYPE_LABEL[c.ReportType] ?? c.ReportType}".`,
...
export async function setAlertStatus(session, alertKey, status) {
  ...
  if (existing) {
    ...
    await table.insertRow({
      ...
      AlertKey: alertKey,
      Status: status,
      CreatedAt: toDbDateTime(new Date()),
      UpdatedAt: toDbDateTime(new Date()),
    });
  }
