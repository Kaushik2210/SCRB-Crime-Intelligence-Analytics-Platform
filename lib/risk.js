...
async function loadScopedCases(scopeFilter) {
  ...
  return prisma.caseMaster.findMany({
...
export async function getRiskTiles(scopeFilter) {
  ...
  const [units, subHeads, gravities] = await Promise.all([
    prisma.unit.findMany({ include: { District: true } }),
    prisma.crimeSubHead.findMany(),
    prisma.gravityOffence.findMany(),
  ]);
  ...
  for (const row of rows) {
    ...
    const isRecent = row.CrimeRegisteredDate >= recentCutoff;
    if (isRecent) {
      bucket.recent += 1;
  ...
  for (const [key, bucket] of buckets.entries()) {
    ...
    tiles.push({
      ...
      districtName: unit.District?.DistrictName ?? null,
