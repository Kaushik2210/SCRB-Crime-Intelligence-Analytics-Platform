 ...
 */
/**
 * Pinned to `globalThis` because this module is loaded twice in one process:
 * once by `server.js` (plain Node `require`) and once from inside the Next.js
 * webpack bundle that serves route handlers and Server Components. Those are
 * separate module instances, so a plain `new AsyncLocalStorage()` would give
 * the writer (server.js) and the readers (the app) *different* stores, and
 * every `getCatalystApp()` would throw "No Catalyst app bound". A
 * `Symbol.for` key makes both copies resolve to the same instance.
 */
const CONTEXT_KEY = Symbol.for("ksp.catalystContext");
export const catalystContext =
...
function getRequestCache() {
  // No Catalyst app bound (local demo mode, or any non-Catalyst context) —
  // signal "no cache" so memoizePerRequest just runs the factory directly.
  let app;
  try {
    app = getCatalystApp();
  } catch {
