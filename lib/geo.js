...
export async function getDistrictHotspots(districtId, opts = {}) {
  if (!Number.isInteger(districtId)) {
    throw new Error("getDistrictHotspots: districtId must be an integer");
  }
  ...
  const cases = await zcqlQuery(
    `SELECT ROWID AS CaseMasterID, Latitude, Longitude FROM CaseMaster
     WHERE PoliceStationID IN (SELECT ROWID FROM Unit WHERE DistrictID = ${districtId})
     ${since ? `AND CrimeRegisteredDate >= '${since.toISOString()}'` : ""}`,
    "CaseMaster"
  );
...
export async function getCasesNearPoint(lat, lng, radiusKm) {
  ...
  const cases = await zcqlQuery(`SELECT ROWID AS CaseMasterID, Latitude, Longitude FROM CaseMaster`, "CaseMaster");
