 ...
 * `sinceDays`, when set, restricts clustering to cases registered in that many
 * trailing days — powers the district page's time-range slider.
 */
export async function getDistrictHotspots(districtId, opts = {}) {
  ...
  try {
    const rows = await prisma.$queryRaw(Prisma.sql`
      ...
      FROM (
        ...
        WHERE u."DistrictID" = ${districtId} AND cm.geog IS NOT NULL
          ${since ? Prisma.sql`AND cm."CrimeRegisteredDate" >= ${since}` : Prisma.empty}
      ) sub
    ...
    return getDistrictHotspotsFallback(districtId, eps, minPoints, since);
  }
}
...
async function getDistrictHotspotsFallback(districtId, epsDegrees, minPoints, since) {
  const cases = await prisma.caseMaster.findMany({
    where: {
      PoliceStation: { DistrictID: districtId },
      ...(since ? { CrimeRegisteredDate: { gte: since } } : {}),
    },
